...
int main(int argc, char *argv[])
{
    ...
    // for(j=0; j<N; j++){
    // 	x[j] = (double *)malloc(sizeof(double)*N);
    ...
    for (iter = 1; iter <= MAXITER; iter++)
    {
        ...
        //MPI_Gather(&(x[0][0]), partitition_size * N, MPI_DOUBLE, &(x[rank * partitition_size][0]), partitition_size * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        //MPI_Gather(&(b[0][0]), partitition_size * N, MPI_DOUBLE, &(b[rank * partitition_size][0]), partitition_size * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
